...
function memoria(){const c=nucleo(),pac={ID_INTERNO:'P-FICTICIO',RUT:'11111111-1',NOMBRE:'PACIENTE ORIGINAL',PROXIMO_CONTROL:'',ULTIMO_CONTROL:'2026-09-01',SECTOR:'VERDE',ESTRATIFICACION:'G2'},eventos=[{ID_EVENTO:'EV-ANTERIOR',ID_INTERNO:pac.ID_INTERNO,TIPO_EVENTO:'CONTROL',FECHA_EVENTO:'2026-09-01',FUENTE:'HISTORICO',DESCRIPCION:''}],escritos=[];let actualizar=0;
 c.Captura_v2_usuarioActual=()=> 'ficticio@example.test';c.Form_hoy=()=> '2026-09-17';c.Modelo_buscarPaciente=id=>id===pac.ID_INTERNO?{obj:pac,idx:0}:null;c.Modelo_leerPacientes=()=>[pac];c.Modelo_leerEventos=()=>c.Captura_eventosVigentes_(eventos);c.Modelo_invalidarLecturas=()=>{};c.Modelo_asegurarEsquemaPacientes=()=>({ok:true});c.Modelo_hoja=()=>({getRange:()=>({setValues:v=>{escritos.push(v);c.MODELO_PACIENTE.forEach((x,i)=>{pac[x.campo]=v[0][i];});}})});c.Modelo_filaFisica=()=>2;c.Modelo_filaDesdeObjeto=o=>c.MODELO_PACIENTE.map(x=>o[x.campo]??'');c.Estrat_recalcularPaciente=()=>({ok:true});c.api_patologiasGuardar=(id,codigos,otras)=>{pac.CONDICIONES=codigos.join(';');pac.OTRAS_PATOLOGIAS=otras;return{ok:true};};c.api_actualizarPaciente=(id,campos)=>{actualizar++;Object.assign(pac,campos);return{ok:true};};c.Captura_v2_marcaEnEventos=marca=>{const e=eventos.find(e=>e.FUENTE===marca);return e?{idInterno:e.ID_INTERNO,idEvento:e.ID_EVENTO}:null;};
 c.api_registrarEvento=q=>{eventos.push({ID_EVENTO:'EV-NUEVO-'+eventos.length,ID_INTERNO:q.idInterno,TIPO_EVENTO:q.tipoEvento,FECHA_EVENTO:q.fecha,FUENTE:q.fuente,DESCRIPCION:q.descripcion});return{ok:true};};
 return {c,pac,eventos,escritos,get actualizar(){return actualizar;}};}
...
test('Fallo de auditoría se recupera por captureId sin duplicar la atención',()=>{
  ...
  const registros=new Map(),real=m.c.api_registrarEvento,marca=m.c.Captura_v2_marca(validar(m.c,entrada).normalizado);let fallar=true;
  m.c.api_registrarEvento=q=>{if(q.fuente===marca&&fallar){fallar=false;return{ok:false,motivo:'AUDITORIA_TEMPORAL'};}return real(q);};
